...
function home(){
  ...
  app.innerHTML=`
    <section class="home-hero">
      <div class="hero-copy">
        <div class="kicker">PERSONAL TRAINING CAMPAIGN</div>
        <h1>PAINT AT YOUR OWN PACE.</h1>
        <p>
          没有 Day 1、Day 2，也没有连续打卡压力。<br>
          选择当前战役里的任意 Mission，慢慢完成。<br>
          只有完成整个战役，下一章才会解锁。
        </p>
      </div>
    </section>
    ...
    <section class="dashboard">
      ${metric('◎','OVERALL PROGRESS','总体进度',pct+'%',`<div class="progress"><div style="width:${pct}%"></div></div>`)}
      ${metric('▤','MISSIONS','任务',done+' / '+total)}
      ${metric('XP','XP EARNED','经验值',xp())}
      ${metric('A','CAMPAIGNS CLEARED','通关战役',cleared+' / '+COURSES.length)}
    </section>
  ...
  document.querySelectorAll('.campaign-banner:not(.locked)').forEach(el=>{
    el.onclick=()=>location.hash='#/campaign/'+COURSES[+el.dataset.i].id;
  });
}

function metric(icon,en,zh,val,extra=''){
  return `<article class="metric">
    <div class="metric-top">
      <div class="metric-icon">${icon}</div>
      <div><label>${en}<span>${zh}</span></label><strong>${val}</strong></div>
    </div>${extra}
  </article>`;
}

function campaignBanner(c,i){
  const u=unlocked(i), d=c.missions.filter(m=>complete(m.id)).length, pct=Math.round(d/c.missions.length*100);
  const cls=i===0?'recruit':i===1?'battle-brother':'generic';
  const status=cdone(i)?'CLEARED / 已通关':u?'MISSIONS / 任务':'LOCKED / 未解锁';
  return `<article class="campaign-banner ${cls} ${u?'':'locked'}" data-i="${i}">
    <div class="banner-flag"><span>☠</span><strong>${c.roman}</strong></div>
    <div class="eyebrow">CAMPAIGN ${c.roman}</div>
    <h2>${c.title} <span class="zh">· ${c.title_zh}</span></h2>
    <p>${c.subtitle_en}<br>${c.subtitle_zh}</p>
    <div class="banner-right">
      ${u?'':`<div class="lock">🔒</div>`}
      <strong>${u?`${d} / ${c.missions.length}`:'LOCKED'}</strong>
      <span>${status}</span>
      ${u?`<div class="chev">›</div>`:`<span style="margin-top:12px;color:#aaa">Complete Campaign ${i} to unlock<br>完成上一战役以解锁</span>`}
    </div>
  </article>`;
}
